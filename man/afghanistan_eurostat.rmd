...
bar_compare_dec_pop + theme_bw() + theme(legend.position="bottom")
```
...
dec_bar +  scale_fill_manual(values = dec_palette_grey) + theme_bw()
```

looking at the percentages:

```{r fill_hist}
dec_bar_fill <- ggplot(filter(major_geo, decision != "TOTAL" & decision != "TOTAL_POS")) + geom_col(aes(x=geo, y=values, fill=decision), colour="black", position="fill") + scale_fill_grey() + theme_bw()
dec_bar_fill
```
