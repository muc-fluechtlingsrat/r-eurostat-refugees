...
```{r reduce_data_total}
    major_geo_total=filter(migr_asydcfsta, values > 500, time == "2017-01-01", decision == "TOTAL", sex == "T", age == "TOTAL", citizen == "AF", geo != "EU28", geo != "TOTAL") %>% 
  select(geo,values) %>%
  arrange(desc(values))
```
...
```{r other_geo_total}
    other_geo_total=filter(migr_asydcfsta, values <= 500, time == "2017-01-01", decision == "TOTAL", sex == "T", age == "TOTAL", citizen == "AF", geo != "EU28", geo != "TOTAL") %>%  arrange(desc(values))
    sum(other_geo_total$values)
```
...
all_geo_total <- rbind(major_geo_total, c("XX",580))
...
```{r reduce_data}
major_geo <-filter(migr_asydcfsta, values > 500, time >= "2017-01-01", sex == "T", age == "TOTAL", citizen == "AF", geo != "EU28", geo != "TOTAL") %>% 
