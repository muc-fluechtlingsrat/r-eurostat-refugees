...
kable(geodic %>% filter(code %in% geo_migr_asydcfsta_major), caption = "Ländercodes der wichtigsten Antragsländer")
...
bar_compare_dec_pop <- ggplot(filter(compare_dec_pop, !geo %in% unlist(drop_countries_comp)),  aes(x=geo, y=values, fill=stat)) + geom_bar(stat="identity", position="dodge") +  ggtitle("Vergleich von Bevölkerung in Tsd zu Asylentscheidungen", subtitle=" zuAfghanistan, 2017; EU Länder mit mehr als 8 Mio Einwohnern") + theme_bw() + theme(legend.position="bottom") + scale_fill_grey(start = 0.4, end = 0.7)
