---
title: "Plots and Analysis around the Article Asylentscheidungen in Europa" 
author: "ynux"
date: "August, 2018"
output: pdf_document
---
...
Andere Länder, andere Aspekte 

#### Dieselbe Analyse wie für Afghanistan für Syrien, Pakistan, Türkei, Somalia, Nigeria, TOTAL
...
kable(major_geo_total_sy[order("values"),], caption = paste("Länder mit mehr als ", cutoff, " Entscheidungen zu Asylanträgen aus ", COUNTRY ))
...
major_geo_sy <-filter(migr_asydcfsta, geo %in% major_geo_total_sy$geo, time >= "2017-01-01", sex == "T", age == "TOTAL", citizen == "SY", geo != "EU28", geo != "TOTAL") %>% 
...
library(knitr)
# Die ersten paar Spalten
kable(major_geo_total_pk[order("values")], caption = paste("Länder mit mehr als ",cutoff, " Entscheidungen zu Asylanträgen aus " , COUNTRY))
major_geo_total_pk <- filter(major_geo_total_pk, geo != "TOTAL")
```

```{r other_geo_total_pk,  echo=FALSE}
    other_geo_total_pk=filter(migr_asydcfsta, values <= cutoff, time == "2017-01-01", decision == "TOTAL", sex == "T", age == "TOTAL", citizen == "PK", geo != "EU28", geo != "TOTAL") %>%  arrange(desc(values))
...
major_geo_pk <-filter(migr_asydcfsta, geo %in% major_geo_total_pk$geo, time >= "2017-01-01", sex == "T", age == "TOTAL", citizen == "PK", geo != "EU28", geo != "TOTAL") %>% 
  select(geo,decision,values) 
