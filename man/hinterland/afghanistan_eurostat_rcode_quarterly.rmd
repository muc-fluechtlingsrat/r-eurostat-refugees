...
#saveRDS(migr_asydcfstq, file="/home/suny/migr_asydcfstq_20180922.rds")
...
dec_bar_de_pct <- ggplot(migr_asydcfstq_de_af) + aes(x=time, y=values, fill=decision) + geom_bar(stat="identity", , position="fill") + scale_fill_manual(values = dec_palette_grey) + scale_x_date(breaks = date_breaks("1 year"),labels = date_format("%Y")) + theme_bw() + theme(legend.position="bottom") + ggtitle("Entscheidungen über Asylanträge aus Afghanistan in Deutschland", subtitle = "vierteljährlich, in Prozent")
dec_bar_de_pct
```
...
dec_bar_se_pct <- ggplot(migr_asydcfstq_se_af) + aes(x=time, y=values, fill=decision) + geom_bar(stat="identity", , position="fill") + scale_fill_manual(values = dec_palette_grey)+ scale_x_date(breaks = date_breaks("1 year"),labels = date_format("%Y")) + theme_bw() + theme(legend.position="bottom") + ggtitle("Entscheidungen über Asylanträge aus Afghanistan in Schweden", subtitle = "vierteljährlich, in Prozent")
dec_bar_se_pct
```
